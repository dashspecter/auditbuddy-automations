-- Block task completion before shift start time
-- Adds shift-start enforcement to complete_task_guarded RPC
...
BEGIN
  ...
  -- 5. NEW: Block completion before shift start time (unless manager override)
  IF NOT p_is_manager_override THEN
    -- Find earliest approved/confirmed shift for this employee on occurrence_date
    SELECT (p_occurrence_date::TEXT || ' ' || s.start_time::TEXT)::TIMESTAMPTZ
