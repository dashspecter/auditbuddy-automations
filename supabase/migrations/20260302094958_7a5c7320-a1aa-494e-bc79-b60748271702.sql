...
-- Drop the orphaned 4-parameter overload
DROP FUNCTION IF EXISTS public.calculate_location_performance_scores(UUID, UUID, DATE, DATE);

-- Recreate the 3-parameter version with fixes:
-- 1. Fix EXTRACT(EPOCH FROM integer) bug in warning penalty
-- 2. Fix punctuality_used to use v_shifts_worked instead of v_shifts_scheduled
...
BEGIN
  ...
  LOOP
    ...
    -- ==================== WARNINGS ====================
    -- FIX: Use integer day arithmetic instead of broken EXTRACT(EPOCH FROM integer)
    v_warning_penalty := 0;
    v_warning_count := 0;
    ...
    SELECT COUNT(*), COALESCE(SUM(
      CASE
        ...
        ELSE 5
      END * GREATEST(0, 1.0 - (v_today - se.event_date::date)::numeric / 90)
    ), 0)
