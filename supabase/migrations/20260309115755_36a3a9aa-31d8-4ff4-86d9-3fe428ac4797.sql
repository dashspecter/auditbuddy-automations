...
-- Update calculate_location_performance_scores to exclude excused (approved) late exceptions from punctuality
...
BEGIN
  ...
  LOOP
    ...
    -- PUNCTUALITY: exclude lates that have an approved exception
    ...
    LOOP
      ...
      LOOP
        ...
        IF v_task_rec.recurrence_type IS NULL OR v_task_rec.recurrence_type = 'none' THEN
          IF v_task_rec.task_created_at::date = v_shift_day.shift_date THEN
            v_task_occurs := true;
          END IF;
            ...
            IF v_task_time >= v_shift_day.emp_shift_start AND v_task_time < v_shift_day.emp_shift_end THEN
              ...
              IF v_eligible_count > 0 THEN
                ...
                WHERE tc.task_id = v_task_rec.id
                  AND tc.employee_id = v_emp.id
                  AND tc.completed_date = v_shift_day.shift_date
                  AND (v_time_slot = '00:00:00' OR tc.time_slot = v_time_slot);

                IF v_completion_count > 0 THEN
                  v_individual_fair_share := (1.0 / v_eligible_count);
                  v_fair_share_total := v_fair_share_total; -- already added above
                END IF;
              END IF;
            END IF;
          END LOOP;
      ...
      LOOP
        ...
        IF v_task_occurs THEN
          ...
          LOOP
            ...
            SELECT COUNT(*) INTO v_completion_count
            FROM task_completions tc
            WHERE tc.task_id = v_task_rec.id AND tc.employee_id = v_emp.id
              AND tc.completed_date = v_shift_day.shift_date
              AND (v_time_slot = '00:00:00' OR tc.time_slot = v_time_slot);
            IF v_completion_count > 0 THEN
    ...
    LEFT JOIN task_locations tl ON tl.task_id = t.id
    WHERE tc.employee_id = v_emp.id
      AND tc.completed_date >= p_start_date AND tc.completed_date <= p_end_date
