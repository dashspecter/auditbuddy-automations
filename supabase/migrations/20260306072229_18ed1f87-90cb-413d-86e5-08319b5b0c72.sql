...
DECLARE
  ...
  v_daily_trend JSON;
BEGIN
  -- Calculate KPIs (now with normalized weight and items count)
  SELECT json_build_object(
    ...
    'total_weight_kg_normalized', COALESCE(SUM(
      CASE 
        WHEN p.uom = 'g' THEN e.weight_kg / 1000.0
        WHEN p.uom IN ('kg', 'l') THEN e.weight_kg
        ELSE 0
      END
    ), 0),
    'total_items_count', COALESCE(SUM(
      CASE 
        WHEN p.uom IN ('pcs', 'portions') THEN e.weight_kg
        ELSE 0
      END
    ), 0)
  ) INTO v_kpis
