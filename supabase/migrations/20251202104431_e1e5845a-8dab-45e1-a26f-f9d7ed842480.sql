-- Fix the reject and approve functions to use correct role checks

-- Drop existing functions
DROP FUNCTION IF EXISTS public.reject_shift_assignment(uuid);
DROP FUNCTION IF EXISTS public.approve_shift_assignment(uuid);

-- Recreate reject function with correct role checks
...
BEGIN
  ...
  -- Check if user is manager or admin (company_admin or company_owner)
  IF v_user_company_role NOT IN ('company_admin', 'company_owner') THEN
...
-- Recreate approve function with correct role checks
