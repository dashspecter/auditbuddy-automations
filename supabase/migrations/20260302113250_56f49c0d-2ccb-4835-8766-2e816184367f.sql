...
-- Step 1: Create task_occurs_on_date helper function
...
-- Step 2: Replace calculate_location_performance_scores with fair share logic
  ...
  LOOP
    ...
    LOOP
      ...
      LOOP
        ...
        ELSE
          IF (v_task_rec.task_created_at AT TIME ZONE 'Europe/Bucharest')::DATE = v_shift_day.shift_date THEN
            v_task_occurs := TRUE;
          ELSE
              ...
              WHERE s2.location_id = p_location_id AND s2.shift_date = v_shift_day.shift_date
                ...
                AND (
                  ...
                  OR lower(trim(translate(
                    COALESCE(
                      (SELECT er3.name FROM employee_roles er3 WHERE er3.id = v_task_rec.assigned_role_id),
                      (SELECT er4.name FROM task_roles tr4 JOIN employee_roles er4 ON er4.id = tr4.role_id WHERE tr4.task_id = v_task_rec.id LIMIT 1),
                      ''
                    ), 'ăâîșțĂÂÎȘȚ', 'aaistsAIST'))) = 
    ...
    -- Shared task completions (unchanged)
    ...
    -- Individual task completions (unchanged)
    SELECT COUNT(*), COUNT(*) FILTER (WHERE COALESCE(tc.completed_late, false) = false)
