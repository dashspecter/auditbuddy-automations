...
import { Plus, Repeat, Edit, Trash2, Copy } from 'lucide-react';
...
const RecurringAuditSchedules = () => {
  ...
  const { audit, audits, location } = useTerminology();
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold">{`Recurring ${auditLabel} Schedules`}</h1>
            <p className="text-muted-foreground">{`Automatically schedule ${auditsLabelLower} on a recurring basis`}</p>
          </div>
        </div>
          ...
          {!schedules || schedules.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground mb-4">
                {`Create your first recurring schedule to automatically generate ${auditsLabelLower}`}
              </p>
              <Button onClick={handleCreate}>
                <Plus className="h-4 w-4 mr-2" />
                Create Schedule
                ...
                <TableBody>
                  {schedules.map((schedule) => (
                    <TableRow key={schedule.id}>
                      ...
                      <TableCell>{schedule.profiles?.full_name || schedule.profiles?.email}</TableCell>
                      <TableCell>
                        <Badge variant="outline">{getRecurrenceLabel(schedule)}</Badge>
                      </TableCell>
                      <TableCell>
      ...
      <RecurringScheduleDialog open={dialogOpen} onOpenChange={setDialogOpen} schedule={editingSchedule} />
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {`Are you sure you want to delete this recurring schedule? This will not affect already scheduled ${auditsLabelLower}.`}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
