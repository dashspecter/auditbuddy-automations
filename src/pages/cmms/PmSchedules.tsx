...
export default function PmSchedules() {
  ...
  const filteredPlans = pmPlans?.filter(p =>
    ...
    p.asset?.name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  return (
    <AppLayout>
      <div className="p-6 space-y-6">
        ...
        {/* PM Plans Table */}
        {isLoading ? (
          <div className="text-center py-12 text-muted-foreground">Loading PM plans...</div>
        ) : filteredPlans.length === 0 ? (
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-12 text-center">
              <Calendar className="h-12 w-12 text-muted-foreground mb-4" />
              <h3 className="text-lg font-semibold mb-2">No preventive maintenance plans yet</h3>
              <p className="text-muted-foreground mb-4 max-w-sm">
                Create PM plans to automatically generate work orders on schedule.
              </p>
              <Button onClick={() => setShowNewDialog(true)}>
                <Plus className="h-4 w-4 mr-2" />
                New PM Plan
            ...
            <Table>
              ...
              <TableBody>
                {filteredPlans.map((plan) => {
                  const dueStatus = getDueStatus(plan.next_due_at);
                  return (
                    <TableRow key={plan.id}>
                      <TableCell className="font-medium">{plan.name}</TableCell>
                      <TableCell>
                        {plan.scope_type === 'asset' && plan.asset && (
                          <span>{plan.asset.name}</span>
                        )}
                        {plan.scope_type === 'location' && plan.location && (
                          <span>{plan.location.name}</span>
                        )}
                        {plan.scope_type === 'category' && plan.category && (
                      ...
                      <TableCell>
                        <div className="flex items-center gap-1.5">
                          ...
                          {getFrequencyLabel(plan.frequency_type, plan.frequency_value)}
                        </div>
                      </TableCell>
                      <TableCell>
                        {plan.next_due_at ? (
                          <Badge
                            variant={
                              ...
                              dueStatus === 'today' ? 'default' :
                              'secondary'
                            }
                          >
                            {dueStatus === 'overdue' && 'Overdue: '}
                            {dueStatus === 'today' && 'Today'}
                            {dueStatus === 'upcoming' && format(new Date(plan.next_due_at), 'MMM d, yyyy')}
                            {dueStatus === 'overdue' && format(new Date(plan.next_due_at), 'MMM d')}
                          </Badge>
                        ) : (
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem 
                              ...
                              onClick={() => deletePmPlan.mutate(plan.id)}
                            >
