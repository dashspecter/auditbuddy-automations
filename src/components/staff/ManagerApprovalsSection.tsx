...
export const ManagerApprovalsSection = () => {
  ...
  const managerLabel = term.manager();
        ...
        <CollapsibleTrigger className="w-full p-4 hover:bg-accent/5 transition-colors">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-semibold">{managerLabel} Approvals</h2>
            </div>
            <div className="flex items-center gap-2">
        ...
        <CollapsibleContent>
          <div className="px-4 pb-4 space-y-4">
                ...
                {exceptions.map((exception) => (
                  <Card key={exception.id} className="p-3">
                    ...
                    <div className="flex gap-2 mt-3">
                      <Button size="sm" className="flex-1" onClick={() => handleResolveException(exception.id, 'approved')} disabled={processingId === exception.id}>
                        <CheckCircle className="h-4 w-4 mr-1" /> Approve
                      </Button>
                      <Button size="sm" variant="outline" className="flex-1" onClick={() => handleResolveException(exception.id, 'denied')} disabled={processingId === exception.id}>
            ...
            {pendingShifts.length > 0 && (
              <div className="space-y-2">
                <h3 className="text-sm font-medium text-muted-foreground">{shiftLabel} Assignments</h3>
                  ...
                  <Card key={assignment.id} className="p-3">
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex-1">
                        ...
                        <Badge variant="outline" className="mt-1 text-xs">{assignment.shifts?.role}</Badge>
                    ...
                    <div className="flex gap-2 mt-3">
                      <Button size="sm" className="flex-1" onClick={() => handleApproveShift(assignment.id)} disabled={processingId === assignment.id}>
                        <CheckCircle className="h-4 w-4 mr-1" /> Approve
                      </Button>
                      <Button size="sm" variant="outline" className="flex-1" onClick={() => handleRejectShift(assignment.id)} disabled={processingId === assignment.id}>
                ...
                {pendingTimeOff.map((request) => {
                  ...
                  return (
                    <Card key={request.id} className="p-3">
                      ...
                      <div className="flex gap-2 mt-3">
                        <Button size="sm" className="flex-1" onClick={() => handleApproveTimeOff(request.id)} disabled={processingId === request.id}>
                          <CheckCircle className="h-4 w-4 mr-1" /> Approve
                        </Button>
                        <Button size="sm" variant="outline" className="flex-1" onClick={() => handleRejectTimeOff(request.id)} disabled={processingId === request.id}>
                          <XCircle className="h-4 w-4 mr-1" /> Reject
                        </Button>
                      </div>
                    </Card>
            ...
            {swapRequests.length > 0 && (
              <div className="space-y-2">
                <h3 className="text-sm font-medium text-muted-foreground">{shiftLabel} Swap Requests</h3>
                {swapRequests.map((swap: any) => (
                  <Card key={swap.id} className="p-3">
                    ...
                    <div className="mb-3 pb-3 border-b">
                      ...
                      <Badge variant="outline" className="mt-1 text-xs">{swap.requester?.shifts?.role}</Badge>
                    </div>
                    ...
                    <div className="mb-3">
                      ...
                      <Badge variant="outline" className="mt-1 text-xs">{swap.target?.shifts?.role}</Badge>
                    </div>

                    <div className="flex gap-2">
                      <Button size="sm" className="flex-1" onClick={() => handleApproveSwap(swap.id)} disabled={processingId === swap.id}>
                        <CheckCircle className="h-4 w-4 mr-1" /> Approve
                      </Button>
                      <Button size="sm" variant="outline" className="flex-1" onClick={() => handleRejectSwap(swap.id)} disabled={processingId === swap.id}>
                        <XCircle className="h-4 w-4 mr-1" /> Reject
                      </Button>
                    </div>
                  </Card>
